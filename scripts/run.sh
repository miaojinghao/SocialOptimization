...
HADOOP_OPTS="-conf /home/btdev/conf/core-prod.xml -Dmapreduce.reduce.memory.mb=8192 -Dmapreduce.reduce.java.opts=-Xmx4096m -Dmapreduce.map.java.opts=-Xmx2048m"
$HADOOP_CMD jar $JARFILE $JARCLASS $HADOOP_OPTS -c $CONFFILE -l $LOGCONFFILE >$LOGFILE

# Save to S3
HADOOP_OPTS="-conf /home/btdev/conf/core-prod.xml -Dmapred.job.queue.name=bt.daily -Ddistcp.bytes.per.map=102400 -overwrite"
while [ $NDAYS -gt 0 ]
do
